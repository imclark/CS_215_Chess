        ...
        public void actionPerformed(ActionEvent e) {
            if (lastTile == null) {
                if (board.isOccupiedByPlayer(thisTile, turn)) {
                    ...
                    for (Tile tile : board.getPieceAt(thisTile).getAllSafeMoves(thisTile)) {	// only displays safe moves 
                ...
                if (board.move(lastTile, thisTile)) {
                    pauseTimers();
                    turn = turn.opposite();                    
                    if(board.isPlayerInCheckMate(turn)){			// So if the requirements are met for being in check mate  
                    	refresh();									// refresh so we can see that we're in check mate
                    	JOptionPane.showMessageDialog(GUIChess.this, "Check Mate! \n " + turn.opposite() + " Wins!" );	// display the message and who won
                    	newGame();									// create a new game after
                    }
                    else if(board.isPlayerInCheck(turn)){			// if the player is in check
                    	refresh();									// refresh so we can see we're in check
                    	JOptionPane.showMessageDialog(GUIChess.this, "Player " + turn + " is in check!");	// Display the check message
                    }
                    resumeTimer(); 			// moved the resumeTimer here so while the messages are displayed the timers don't continue
                }               
                refresh();           
                lastTile = null;                
            }
